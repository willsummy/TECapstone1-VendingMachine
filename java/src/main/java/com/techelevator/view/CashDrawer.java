...
public class CashDrawer {
    ...
    /*
    constructor, set balance to 0


    addMoney(double input)
    this.balance += input;

    getBalance()


    validatePrice(slotID) {

    double price = inventory.getProductPrice(slotID)
    if price > balance
        return false
    ...
    returnChange() {
    least coins possible with nickels, dimes, quarters
    balance = 0
    }
    ...
    ui select product will send slotID
