...
public class UserInterface {
    ...
    sout - display items, purchase, exit
        getUserInput() -  parse to int 1, 2, or 3 (if 3, exit -- if 2, call purchaseMenu())
        validate userInput
           if 1
                displayItems()
                continue
    ...
    getUserInput() - asks for String

    displayItems() - displays menu if first user input is 1  - inventoryObject.getInventory
     ...
     - public void purchaseMenu() {
        while (true) {
                ...
                if 2
                    ...
                    check if valid
                    if valid
                        boolean validPrice = validatePrice(slotID)
                        boolean validStock = validateStock(slotID)
                        if (!validPrice) sout "not enough money"
                        else if (!validStock) sout "not in stock"
                        else grabProduct'
                            Product product = inventory.grabProduct(slotID)
                            cashDrawer.subtractPrice()
                            sout product.getName + get cost + cashDrawer.getBalance;
                            sout productObject custom toStrin (eating)
                        add to Log.txt
                   if not valid - scold user

                        continue
                if 3
                    finish transaction
                    return change - sout change
                    writes transaction to Log.txt
                    return to main menu
